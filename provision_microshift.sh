...
run_playbook() {
  local mode="$1"
  local playbook="${SCRIPT_DIR}/plays/main.yaml"
  local tags=""
  local instance_ids=""

  case "$mode" in
    "provision")
      playbook="${SCRIPT_DIR}/plays/provision-vm.yaml"
      ;;
    "deprovision")
      playbook="${SCRIPT_DIR}/plays/deprovision-vm.yaml"
      ;;
    "cleanup")
      playbook="${SCRIPT_DIR}/plays/cleanup-old.yaml"
      echo "cleanup mode"
      ;;
    *)
